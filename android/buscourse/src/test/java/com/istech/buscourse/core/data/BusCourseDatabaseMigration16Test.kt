...
class BusCourseDatabaseMigration16Test {
    ...
    fun roomOpensActualV15DatabaseAfterMigrationAndDaoQuerySucceeds() = runTest {
        ...
        helper.writableDatabase.apply {
            ...
            // v19 で course_stop に4列が増えたため（MIGRATION_17_19。当初は v18/MIGRATION_17_18 として
            // 実装したが二重鋳造が判明し官房裁定で v19 へリナンバー・2026-07-30）、fixture 側も v15 相当へ戻す。
            // 戻さないと MIGRATION_17_19 が同じ列を足そうとして duplicate column name で落ちる。
            //
    ...
    private fun downgradeCourseStopToBeforeV19(db: SupportSQLiteDatabase) {
        ...
        var ddl = createSql
        V19_COURSE_STOP_ADDED_COLUMNS.forEach { col ->
            ddl = ddl.replace(Regex(",\\s*`$col`[^,)]*"), "")
        }
        check(V19_COURSE_STOP_ADDED_COLUMNS.none { ddl.contains("`$it`") }) {
    ...
    private companion object {
        ...
        /** v19（MIGRATION_17_19）で course_stop に純増した4列。fixture を遡らせるときに落とす。 */
        val V19_COURSE_STOP_ADDED_COLUMNS =
