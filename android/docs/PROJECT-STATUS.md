...
- 最新リリース: **`v0.3.3-beta`**（署名済み Lite/Pro APK 配布済み）。診断ビルド **`v0.3.4-beta`** をCI通過後に発行予定（下記の取り込み調査用）。
- **βテスト継続中**。直近で解消済み: 8-5 カテゴリメニューのスクロール、6-5 黒塗り範囲指定プレビュー、書き出し系（overlay化で0バイト解消）、メモ例文言、アプリ内バージョン表示追加。
- **要調査（最優先）**: **Lite→Pro 移行取り込みが0件**。静的解析ではロジック上取り込めるはずで原因未確定。→ `v0.3.4-beta` に**診断計測**を投入（書き出し「画像N枚」／取り込み「entry/img/manifest」内訳をトースト表示）。次のテストで故障箇所を確定し本修正する予定。
- 適用済みの関連修正: 移行取り込みのファイル名フォールバック、バックアップ復元の重複判定をライブのみに変更（削除画像の復元可）、recentsのON_PAUSE即ロック、出力ファイル名を `Lite移行-` / `Proバックアップ-` 始まりに。
