...
> 別モデルで脅威モデリング済み。**確定仕様＝`docs/2026-07-04_仕様_提出用出力機能.md`（v1.1）**。検討書との差分（調整1・2）は同仕様書 §11 に記録。**現時点で実装コードは無し**。
...
- **実装計画**: `docs/2026-07-04_実装計画_提出用出力機能.md`（作成済み。Phase0土台→1ログ→2印刷本体→3テスト/CI→4docs）。**着手前の要決定 Q1–Q4**（ログ統合/設定保存/隠し解除ジェスチャ/範囲UI）は計画§8。**実装は承認後**。※検討書原文・仕様v1.0は履歴として別途コミット可（ユーザー保有）。
